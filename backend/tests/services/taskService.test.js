...
describe("taskService", () => {
  ...
  describe("getTaskByGroup", () => {
    it("should return all tasks in a group", async () => {
      const groupName = "validGroupName";
      Task.find.mockResolvedValueOnce([
        { title: "Task 1" },
        { title: "Task 2" },
      ]);
      const tasks = await taskService.getTaskByGroup(groupName);
      expect(tasks).toEqual([{ title: "Task 1" }, { title: "Task 2" }]);
    });
    it("should return null if no tasks are found in a group", async () => {
      const groupName = "invalidGroupName";
      Task.find.mockResolvedValueOnce(null);
      const tasks = await taskService.getTaskByGroup(groupName);
